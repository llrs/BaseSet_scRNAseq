...
Note that there is the recommendation to see individually which is the appropiate threshold.ç
...
```{r tidyAssigned}
TS2 <- tidySet(cellsAssigned) %>% 
  add_elements(colnames(AUC)) # To have all the missing cells
nElements(TS2) # The original number of cells
es2 <- element_size(TS2)
es2 %>%
  count(size, sort = TRUE)
```

Using the automatically assigned cell types bye AUCell we can see that they cross the thresholds for several cells too.

This might be explained by the comment at the end of the vignette:
...
Maybe now the question you might have is how would I assign each cell to each cell type.

Firstly I wouldn't have any problem  with assigning the one that AUCell would suggest if 

- it were the one with the highest value and 
- the probability for said cell to have just one cell type would be the highest.

Otherwise, the assignment is mostly incorrect as is done with an arbitrary threshold and without considering all the information available.

In this case with just 6 cell types the assignment is mostly as an example but with hundreds or thousands of cells the problem becomes more apparent
